...
def _(df):
    ...
    target_column = "data.jv.default_PCE"
...
def _(METRIC_LABELS, paths, plot_performance_comparison, results_default):
    perovskite_labels = METRIC_LABELS.copy()
    perovskite_labels["mae"] = "MAE (% PCE)"

    fig_performance = plot_performance_comparison(
        results_default,
        target_type="regression",
        metric_labels=perovskite_labels,
        save_path=paths.figures / "perovskite_performance_comparison.pdf",
        title_suffix="Perovskite Dataset",
    )
    fig_performance.show()
    return fig_performance, perovskite_labels
...
def _(paths, plot_meta_performance, results_default):
    fig_meta = plot_meta_performance(
        results_default,
        save_path=paths.figures / "perovskite_meta_performance.pdf",
        title_suffix="Perovskite Dataset",
    )
    fig_meta
    return
...
@app.cell
def _(paths, perovskite_labels, plot_parameter_sweep_results, sweep_results):
    fig_sweep = plot_parameter_sweep_results(
        sweep_results,
        metric="mae",
        metric_labels=perovskite_labels,
        save_path=paths.figures / "perovskite_parameter_sweep.pdf",
        title_suffix="Perovskite Dataset",
    )
    fig_sweep
    return
