...
def _(df):
    ...
    df_final = df.dropna(
        subset=["authors_full_list", "data.jv.default_Jsc"] + features
    )

    print(f"Dataset shape: {df_final.shape}")
    print(f"Number of features: {len(features)}")

    target_column = "data.jv.default_Jsc"
...
def _(METRIC_LABELS, paths, plot_performance_comparison, results_default):
    perovskite_jsc_labels = METRIC_LABELS.copy()
    perovskite_jsc_labels["mae"] = "MAE (mA/cm²)"

    fig_performance = plot_performance_comparison(
        results_default,
        target_type="regression",
        metric_labels=perovskite_jsc_labels,
        save_path=paths.figures / "perovskite_jsc_performance_comparison.pdf",
        title_suffix="Perovskite Jsc Dataset",
    )
    fig_performance.show()
    return fig_performance, perovskite_jsc_labels
...
def _(paths, plot_meta_performance, results_default):
    fig_meta = plot_meta_performance(
        results_default,
        save_path=paths.figures / "perovskite_jsc_meta_performance.pdf",
        title_suffix="Perovskite Jsc Dataset",
    )
    fig_meta
    return
...
@app.cell
def _(paths, perovskite_jsc_labels, plot_parameter_sweep_results, sweep_results):
    fig_sweep = plot_parameter_sweep_results(
        sweep_results,
        metric="mae",
        metric_labels=perovskite_jsc_labels,
        save_path=paths.figures / "perovskite_jsc_parameter_sweep.pdf",
        title_suffix="Perovskite Jsc Dataset",
    )
    fig_sweep
    return
...
@app.cell
def _(create_main_figure_panel, paths, perovskite_jsc_labels, results_default):
    fig_main = create_main_figure_panel(
        results_default,
        target_type="regression",
        dataset_name="Perovskite Jsc",
        metric_labels=perovskite_jsc_labels,
        save_path=paths.figures / "perovskite_jsc_main_panel.pdf",
    )
    fig_main
    return
