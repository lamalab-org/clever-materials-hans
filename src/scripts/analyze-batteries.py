...
@app.cell
def _(df):
    df_final = df.dropna(subset="authors_full_list")
...
def _(METRIC_LABELS, paths, plot_performance_comparison, results_default):
    battery_labels = METRIC_LABELS.copy()
    battery_labels["mae"] = "MAE"

    fig_performance = plot_performance_comparison(
        results_default,
        target_type="regression",
        metric_labels=battery_labels,
        save_path=paths.figures / "battery_performance_comparison.pdf",
        title_suffix="Battery Dataset",
    )
    fig_performance.show()
    return battery_labels, fig_performance
...
def _(paths, plot_meta_performance, results_default):
    fig_meta = plot_meta_performance(
        results_default,
        save_path=paths.figures / "battery_meta_performance.pdf",
        title_suffix="Battery Dataset",
    )
    fig_meta
    return
...
@app.cell
def _(battery_labels, paths, plot_parameter_sweep_results, sweep_results):
    fig_sweep = plot_parameter_sweep_results(
        sweep_results,
        metric="mae",
        metric_labels=battery_labels,
        save_path=paths.figures / "battery_parameter_sweep.pdf",
        title_suffix="Battery Dataset",
    )
    fig_sweep
    return
...
@app.cell
def _(battery_labels, create_main_figure_panel, paths, results_default):
    fig_main = create_main_figure_panel(
        results_default,
        target_type="regression",
        dataset_name="Battery",
        metric_labels=battery_labels,
        save_path=paths.figures / "battery_main_panel.pdf",
    )
    fig_main
    return
